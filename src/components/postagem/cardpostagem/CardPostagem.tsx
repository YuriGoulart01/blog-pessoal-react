...
function CardPostagem({ postagem }: CardPostagensProps) {
    return(
        <div className="border-slate-900 border flex flex-col rounded overflow-hidden justify-between">
            ...
            <div className="flex">
                <Link to={`/editarpostagem/${postagem.id}`} className="w-full text-white bg-green-600 hover:bg-green-800 flex items-center justify-center py-2"> <button>Editar</button></Link>
                <Link to={`/deletarpostagem/${postagem.id}`} className="w-full text-white bg-red-500 hover:bg-red-800 flex items-center justify-center"> <button>Deletar</button></Link>
            </div>
        </div>
    )
}
